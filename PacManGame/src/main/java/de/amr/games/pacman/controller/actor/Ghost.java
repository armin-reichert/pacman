...
public class Ghost extends Creature<GhostState> {
	...
	public void leavingGhostHouse() {
		Tile exit = world.houses().findFirst().get().seat(0).tile;
	...
	private boolean hasLeftGhostHouse() {
		return tf.y == world.houses().findFirst().get().seat(0).tile.y();
	}
	...
	public Steering isReturningToHouse() {
		return isHeadingFor(world.houses().findFirst().get().seat(0).tile);
	}
