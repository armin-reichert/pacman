...
public interface SteeredMazeMover extends MazeMover {
	...
	/**
	 * Lets the actor follow the shortest path to the target. Depending on the
	 * actor's current state, this path might not be completely accessible for the
	 * actor.
	 * 
	 * @param fnTarget function supplying the target tile
	 * 
	 * @return behavior where an actor follows the shortest (using Manhattan
